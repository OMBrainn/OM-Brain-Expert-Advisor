...
input string TimeFrame = "15M";
input float LiquidityRange = 0.00002f;
input int Window = 100;   // Number of candles to scan
...
double MQLR_Bars(int i, string Request){
   ...
   CopyHigh(Symbol(),PERIOD_CURRENT,0,Window,High);
   
   CopyLow(Symbol(),PERIOD_CURRENT,0,Window,Low);
   
   HighestCandle = ArrayMaximum(High,0,Window);
   LowestCandle = ArrayMinimum(Low,0,Window);
   
   MqlRates PriceInformation[];
   
   ArraySetAsSeries(PriceInformation, true);
   
   int Data = CopyRates(Symbol(), PERIOD_CURRENT, 0, Bars("AUDUSD",PERIOD_CURRENT), PriceInformation);
   //Reuqest
   double return_;
   if(Request == "Time"){
...
int OnInit()
  {
   LastCandleNumber = Bars(_Symbol,PERIOD_CURRENT);
...
void CheckForNewCandle(int CandleNumber) {
   if(CandleNumber > LastCandleNumber){
      ...
      if(!FirstCandleProcess_) {
         Print("First Candle");
         LastCandleNumber = CandleNumber;
         Conditions_N_Exe();
         FirstCandleProcess_ = true;
      }
      else if(FirstCandleProcess_){
         Print("New Candle");
         LastCandleNumber = CandleNumber;
         Conditions_N_Exe();
         CleanUp = true;
...
void OnTick(){
   ...
   if(LiquidityInfoProcess_ && CandleInfoProcess_ && CandleInfoDisplay_ && !CleanUp){
      int CandleNumber = Bars(_Symbol,PERIOD_CURRENT);
